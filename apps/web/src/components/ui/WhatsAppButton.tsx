...
// Always-on, anchored bottom-right. Stacks under the AI Tutor button (which
// uses bottom-24) and the ScrollToTop button (which lives bottom-left).
export default function WhatsAppButton() {
  return (
    <a
      ...
      className="fixed bottom-6 right-6 z-40 w-14 h-14 bg-[#25D366] hover:bg-[#1DA851] rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all hover:scale-110 group text-[#25D366]"
    >
      {/* Continuous wavy ripples — text-[#25D366] above colors the rings
          (ripple-wave uses currentColor). pointer-events: none on the spans
          keeps the click target the anchor itself. */}
