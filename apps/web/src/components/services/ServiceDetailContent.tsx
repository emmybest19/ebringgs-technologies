...
export interface ServiceDetailContentProps {
  ...
  backHref: string;
  backLabel?: string;
  /** When true, render without the dark hero section (suitable for embedding inside a dashboard layout). */
  embedded?: boolean;
}
...
export default function ServiceDetailContent({
  ...
  const { data: service, isLoading: loading } = useService(serviceId);
  const { data: pastExamples = [] } = useCaseStudies(
    serviceId ? { serviceId, type: 'client_work' } : {},
  );
  ...
  if (!service) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <Link to={backHref} className="text-teal-600 font-medium hover:underline">Back to services</Link>
      </div>
    );
  }
  ...
  return (
    <div className={embedded ? '' : 'bg-white dark:bg-slate-900 min-h-screen'}>
      {embedded ? (
        // Embedded header — fits inside the dashboard's light card style.
        <div className="mb-6">
          <Link to={backHref} className="inline-flex items-center gap-1.5 text-gray-500 dark:text-slate-400 hover:text-teal-600 dark:hover:text-teal-400 text-sm mb-4 transition-colors">
            <ArrowLeft size={14} /> {backLabel}
          </Link>
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 bg-teal-50 dark:bg-teal-950 rounded-2xl flex items-center justify-center shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white mt-2 mb-1">{service.title}</h1>
              <p className="text-gray-500 dark:text-slate-400 text-sm">{service.description}</p>
              <div className="flex flex-wrap gap-4 mt-3 text-xs text-gray-500 dark:text-slate-400">
                {service.timeline && (
                  <div className="flex items-center gap-1.5">
                    <Clock size={13} className="text-teal-600" /> {service.timeline}
                  </div>
                )}
                {service.productized && typeof service.price === 'number' && (
                  <div className="flex items-center gap-1.5">
                    <ShieldCheck size={13} className="text-green-500" /> Fixed price
                  </div>
                )}
              </div>
            </div>
        ...
        <section className="bg-linear-to-br from-slate-900 to-teal-950 text-white py-20 px-4">
          <div className="max-w-5xl mx-auto">
            <Link to={backHref} className="inline-flex items-center gap-1.5 text-slate-400 hover:text-white text-sm mb-8 transition-colors">
              <ArrowLeft size={14} /> {backLabel}
            </Link>
                ...
                <>
                  ...
                  <div className="mb-5">
                    <div className="flex items-baseline gap-2">
                      ...
                      <span className="text-teal-200 text-xs font-medium">+ 7.5% VAT</span>
                    </div>
