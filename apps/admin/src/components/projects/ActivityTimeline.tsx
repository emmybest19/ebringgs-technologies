...
const typeMeta: Record<ProjectUpdate['type'], { label: string; icon: typeof Activity; color: string; bg: string }> = {
  progress:   { label: 'Progress',   icon: Activity,    color: 'text-cyan-600',   bg: 'bg-cyan-50 dark:bg-cyan-950' },
    ...
    <div className="bg-white dark:bg-[#0e141c] rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={() => refetch()}
          disabled={isFetching}
          className="text-gray-400 hover:text-cyan-600 disabled:opacity-50 transition-colors"
          title="Refresh"
        >
              ...
              <li key={u._id} className="ml-6">
                ...
                {u.url && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 text-xs text-cyan-600 hover:text-cyan-800 hover:underline mb-1.5"
                  >
